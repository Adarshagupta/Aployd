...
class Index extends Component
{
    ...
    public function mount()
    {
        ...
        // Always generate a fresh key pair for each customer by default
        ['private' => $generatedPrivateKey, 'public' => $generatedPublicKey] = generateSSHKey();

        if (isDev()) {
            // In dev mode, we can use environment variables to override the key
            $this->privateKey = env('DEV_SSH_PRIVATE_KEY', $generatedPrivateKey);
            $this->privateKeyDescription = 'Created by Coolify';
            $this->remoteServerDescription = 'Created by Coolify';
            $this->remoteServerHost = env('DEV_SSH_HOST', 'coolify-testing-host');
        } else {
            // For customers, always use a fresh key
            $this->privateKey = $generatedPrivateKey;
            $this->privateKeyDescription = 'Auto-generated by Coolify';
            $this->remoteServerDescription = 'Created by Coolify';
        }
    }
    ...
    {
        // For development testing only - bypass SSH authentication
        if (isDev() && env('BYPASS_SSH_VALIDATION', false)) {
        ...
        try {
            ...
            if (str_contains($errorMessage, 'Permission denied')) {
                $serverPublicKey = $this->createdServer->privateKey->getPublicKey();

                return $this->dispatch('error', 'SSH authentication failed. Please follow these steps to configure SSH access:
...
For more details, see the [official Coolify documentation on SSH configuration](https://coolify.io/docs/knowledge-base/ssh/openssh).

After completing these steps, try validating again.');
            }
    ...
    /**
     * Display help information about verifying SSH configuration
     */
    public function showSshHelp()
    {
        if (! $this->createdServer || ! $this->createdServer->privateKey) {
            return $this->dispatch('error', 'Server or private key not found.');
        }

        $serverPublicKey = $this->createdServer->privateKey->getPublicKey();
        $this->dispatch('info', 'SSH Configuration Help', 'Follow these steps to configure SSH access:
